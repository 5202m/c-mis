...
var Syllabus = {
  ...
  setEvent: function () {
    ...
    $("#syllabus_queryForm_search").on("click", function () {
      var queryParams = $('#' + Syllabus.gridId).datagrid(
          'options').queryParams;
  ...
  getLecturerList: function () {
    var gtype = $("#syllabusEdit_groupType_select").val(),
         gId = $("#syllabusEdit_groupId_select").val() || $("#syllabusEdit_groupId_select").attr("tv");
  ...
  setCourseByClone: function (timeBucket, courses) {
    ...
    cloneObj.find(".main").each(function () {
      $(this).find(".lecturerDiv").find("select[id^='select_lecturer']").attr("id", ltId + $(this).index());
      loc_day = $(this).attr("dy");
    });
  ...
  setEditEvent: function () {
    ...
    $("#showLinksId").hover(function () {
          ...
          var ret = $('select.liveLink').map(function () {
            if ($(this).attr("t") == 2) {
              return null;
            }
          ...
          if (pmc.find("strong").length == 0 && pmc.length > 1) {
            var hasError = $('#showLinksDiv p label:contains("大陆")').length < lpSize
                && $('#showLinksDiv p label:contains("海外")').length < lpSize;
            if (!hasError) {
    ...
    $("#panel_editSyllabus thead select[name='status']").change(function () {
      ...
      if (disable) {
        txtEl.val("");
        pMain.find("select[id^='select_lecturer']:first").combotree("disable").combotree("clear");
      } else {
        pMain.find("select[id^='select_lecturer']:first").combotree("enable");
      }
    });
    //新增默认状态为无效
    ...
    if (this.coursesData) {
      ...
      $("#panel_editSyllabus table:first").find("thead tr th[dy]").each(
          function () {
            var hasRow = false;
      ...
      success: function (data) {
        ...
        if (loc_syllabus && loc_syllabus.courses) {
          ...
          var loc_currTime = {
            ...
            time: (loc_nowTime.getHours() < 10 ? "0" : "") + loc_nowTime.getHours() + ":" + (loc_nowTime.getMinutes() < 10 ? "0" : "") + loc_nowTime.getMinutes()
          };
          var loc_style = 1;
  ...
  getCoursesJson: function () {
    ...
    if (isValid(allowDayTd)) {
      ...
      var allTime = $panel.find("tbody:visible").map(function () {
        ...
        return {
          startTime: $(this).find("input[name='startTime']").timespinner('getValue'),
          endTime: $(this).find("input[name='endTime']").timespinner('getValue'),
          course: courseArr
        };
      }).get();
      console.log(allTime);
      return JSON.stringify({days: weekArr,timeBuckets: allTime.sort(arraySort("startTime", false))});
    } else {
      return "";
    }
  },
  ...
  viewSyllabus: function (syllabus, currTime, style) {
    ...
    //计算时间状态,返回对应的样式
    var loc_timeClsFunc = function (day, startTime, endTime, currTime,
        notCheckTime) {
      var loc_index = 0;
      ...
      if (syllabus) {
        ...
        if (loc_dayLen > 0) {
          if (style === 1) {
            ...
            //星期（头部）
            var loc_startDateTime = currTime.timeLong - 86400000
                * ((currTime.day + 6) % 7);
            var loc_date = null;
            ...
            for (var i = 0; i < loc_dayLen; i++) {
              loc_date = new Date(loc_startDateTime + ((loc_courses.days[i].day
                  + 6) % 7) * 86400000);
              loc_dateStr = loc_date.getFullYear() + "-"
                  + (loc_date.getMonth() < 9 ? "0" : "") + (loc_date.getMonth()
                  + 1) + "-"
                  + (loc_date.getDate() < 10 ? "0" : "") + loc_date.getDate();
              loc_activeCls = (loc_courses.days[i].day == currTime.day)
                  ? ' class="active"' : "";
              loc_html.push(
                  '<a href="javascript:void(0)" d="' + loc_courses.days[i].day
                  + '"' + loc_activeCls + loc_width + '>'
                  + loc_constants.dayCN[loc_courses.days[i].day] + '<br/></a>');
            }
            loc_html.push('</div>');
            //课程
            ...
            for (var i = 0; i < loc_dayLen; i++) {
              ...
              if (loc_courses.days[i].status == 0) {
                //全天休市
                loc_timeCls = loc_timeClsFunc(loc_courses.days[i].day, null,
                    null, currTime, true);
                loc_html.push('<tr class="' + loc_timeCls
                    + '" width="100%"><td>休市</td></tr>');
              }
              else {
                for (var j = 0, lenJ = loc_courses.timeBuckets.length; j < lenJ;
                    j++) {
                  loc_timeBucket = loc_courses.timeBuckets[j];
                  loc_timeCls = loc_timeClsFunc(loc_courses.days[i].day,
                      loc_timeBucket.startTime, loc_timeBucket.endTime,
                      currTime, false);
                  if (loc_timeBucket.course[i].status == 0) {
                    //时间段休市
                    loc_html.push('<tr class="' + loc_timeCls + '">');
                    loc_html.push(
                        '<td width="25%">' + loc_timeBucket.startTime + '-'
                        + loc_timeBucket.endTime + '</td>');
                    ...
                    loc_html.push(
                        ...
                        + loc_timeBucket.endTime + '</td>');
                    loc_html.push(
                        '<td width="25%">' + loc_timeBucket.course[i].lecturer
                        + '</td>');
                    loc_html.push(
                        '<td width="50%">' + loc_timeBucket.course[i].title
                        + '</td>');
                    loc_html.push('</tr>');
                  }
                }
            ...
            var lenJ = !loc_courses.timeBuckets ? 0
                : loc_courses.timeBuckets.length;
            var loc_timeBucket = null;
            var loc_timeCls = null;
            for (var j = 0; j < lenJ; j++) {
              loc_timeBucket = loc_courses.timeBuckets[j];
              loc_html.push('<th>' + loc_constants.indexCN[j] + "<br><span>("
                  + loc_timeBucket.startTime + "-" + loc_timeBucket.endTime
                  + ')</span></th>');
            }
            loc_html.push('</tr>');
            //课程
            var loc_day = null;
            for (var i = 0, lenI = !loc_courses.days ? 0
                : loc_courses.days.length; i < lenI; i++) {
              ...
              if (loc_day.status == 0) {
                //全天休市
                loc_timeCls = loc_timeClsFunc(loc_day.day, null, null, currTime,
                    true);
                loc_html.push('<td class="' + loc_timeCls + '" colspan="' + lenJ
                    + '">休市</td>');
              }
              else {
                for (var j = 0; j < lenJ; j++) {
                  loc_timeBucket = loc_courses.timeBuckets[j];
                  loc_timeCls = loc_timeClsFunc(loc_day.day,
                      loc_timeBucket.startTime, loc_timeBucket.endTime,
                      currTime, false);
                  loc_html.push('<td class="' + loc_timeCls + '">'
                      + (loc_timeBucket.course[i].status == 0 ? "休市"
                          : loc_timeBucket.course[i].lecturer) + '</td>');
                }
              }
              loc_html.push('</tr>');
            }
